...
const Receipts = () => {
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm('Are you sure you want to delete this receipt?')) {
            try {
                await axios.delete(`http://localhost:5001/Receipts/${selectedReceipt.receipt_id}`);
                alert('Receipt deleted successfully');
                setSelectedReceipt(null); // Clear the selection
                setReceipts(receipts.filter((r) => r.receipt_id !== selectedReceipt.receipt_id)); // Remove from UI
            } catch (error) {
                    ...
                    <tbody>
                        {receipts.map((receipt) => (
                            <tr
                                ...
                                onClick={() => handleRowSelection(receipt)}
                                style={{
                                    backgroundColor:
