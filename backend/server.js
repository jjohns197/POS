...
app.post('/submit-order', (req, res) => {
  const { total_amount, employee_id, customer_id, order_id } = req.body;

  if (!total_amount || !employee_id || !customer_id || !order_id) {
    return res.status(400).json({ error: "Missing required data" });
  }
  ...
  const receiptQuery = `
      INSERT INTO receipts (employee_id, customer_id, receipt_date, total_amt, order_id)
      VALUES (?, ?, ?, ?, ?)
  `;

  db.query(receiptQuery, [employee_id, customer_id, receiptDate, total_amount, order_id], (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: "Failed to create receipt" });
    }

    res.status(201).json({ message: "Order submitted successfully", receiptId: result.insertId });
  });
});
